...
if [ -r "$CONFIG_FILE" ]; then
  ...
  CONFIG_DRAKKAR_API_TOKEN="$(sed -n 's/.*"drakkarApiToken"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$CONFIG_FILE" | head -n 1)"
  if [ -z "$CONFIG_DRAKKAR_API_TOKEN" ]; then
    CONFIG_DRAKKAR_API_TOKEN="$(sed -n 's/.*"frontendApiToken"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$CONFIG_FILE" | head -n 1)"
  fi
...
window.__DRAKKAR_CONFIG__ = {
  ...
  DRAKKAR_API_TOKEN: "$CONFIG_DRAKKAR_API_TOKEN",
  FRONTEND_API_TOKEN: "$CONFIG_DRAKKAR_API_TOKEN",
